...
static void stream_close(FFPlayer *ffp)
{
    ...
    av_freep(&afilters);
    if(vfilters_list){
        ALOGE("nb_vfilters : %d", nb_vfilters);
        ALOGE("vfilters_list[0] : %s", vfilters_list[0]);
        vfilters_list[0] = NULL;
        vfilters_list = NULL;
//        av_freep(&vfilters_list[0]);
    }
#endif
}
...
/* seek in the stream */
static void stream_seek(VideoState *is, int64_t pos, int64_t rel, int seek_by_bytesSeek)
{
    if (!is->seek_req) {
        is->seek_pos = pos;
        is->seek_rel = rel;
        is->seek_flags &= ~AVSEEK_FLAG_BYTE;
        if (seek_by_bytesSeek)
            is->seek_flags |= AVSEEK_FLAG_BYTE;
        is->seek_req = 1;
...
static double vp_duration(VideoState *is, Frame *vp, Frame *nextvp) {
    if (vp->serial == nextvp->serial) {
        double durationVp = nextvp->pts - vp->pts;
        if (isnan(durationVp) || durationVp <= 0 || durationVp > is->max_frame_duration)
            return vp->duration;
        else
            return durationVp;
    } else {
...
retry:
        if (frame_queue_nb_remaining(&is->pictq) == 0) {
            ...
            double last_duration, durationRefresh, delay;
            Frame *vp, *lastvp;
            ...
            if (frame_queue_nb_remaining(&is->pictq) > 1) {
                ...
                durationRefresh = vp_duration(is, vp, nextvp);
                if(!is->step && (redisplay || ffp->framedrop > 0 || (ffp->framedrop && get_master_sync_type(is) != AV_SYNC_VIDEO_MASTER)) && time > is->frame_timer + durationRefresh) {
                    if (!redisplay)
                        is->frame_drops_late++;
                    frame_queue_next(&is->pictq);
                    redisplay = 0;
...
static int queue_picture(FFPlayer *ffp, AVFrame *src_frame, double pts, double durationQueue, int64_t pos, int serial)
{
    VideoState *is = ffp->is;
    Frame *vp;
...
static int configure_audio_filters(FFPlayer *ffp, const char *afiltersCon, int force_output_format)
{
    VideoState *is = ffp->is;
    ...
    afilters_args[0] = 0;
    if (afiltersCon)
        snprintf(afilters_args, sizeof(afilters_args), "%s", afiltersCon);
    ...
    do {
        ...
        if (got_frame) {
                ...
                if (reconfigure) {
                    ...
                    if ((ret = configure_audio_filters(ffp, afilters, 1)) < 0) {
...
static int ffplay_video_thread(void *arg)
{
    ...
    for (;;) {
        ...
        if (   last_w != frame->width
            ...
            if ((ret = configure_video_filters(ffp, graph, is, vfilters_list ? vfilters_list[is->vfilter_idx] : NULL, frame)) < 0) {
                // FIXME: post error
...
#endif
            durationVideo = (frame_rate.num && frame_rate.den ? av_q2d((AVRational){frame_rate.den, frame_rate.num}) : 0);
            pts = (frame->pts == AV_NOPTS_VALUE) ? NAN : frame->pts * av_q2d(tb);
            ret = queue_picture(ffp, frame, pts, durationVideo, av_frame_get_pkt_pos(frame), is->viddec.pkt_serial);
            av_frame_unref(frame);
#if CONFIG_AVFILTER
        }
#endif
...
static int stream_component_open(FFPlayer *ffp, int stream_index)
{
    ...
    case AVMEDIA_TYPE_AUDIO:
#if CONFIG_AVFILTER
        {
            ...
            SDL_LockMutex(ffp->af_mutex);
            if ((ret = configure_audio_filters(ffp, afilters, 0)) < 0) {
                SDL_UnlockMutex(ffp->af_mutex);
                goto fail;
            }
            ffp->af_changed = 0;
...
int ffp_prepare_async_l(FFPlayer *ffp, const char *file_name, int argc, char **argv)
{
    assert(ffp);
    assert(!ffp->is);
    assert(file_name);
...
//    }
    ...
    if(argc > 0){
        ...
        is = stream_open(ffp, file_name, file_iformat);
    }else {
...
int ffp_seek_to_l(FFPlayer *ffp, long msec)
{
    ...
    int64_t start_timeSeek = is->ic->start_time;
    if (start_timeSeek > 0 && start_timeSeek != AV_NOPTS_VALUE)
        seek_pos += start_timeSeek;

    // FIXME: 9 seek by bytes
    // FIXME: 9 seek out of range
    // FIXME: 9 seekable
    av_log(ffp, AV_LOG_DEBUG, "stream_seek %"PRId64"(%d) + %"PRId64", \n", seek_pos, (int)msec, start_timeSeek);
...
long ffp_get_current_position_l(FFPlayer *ffp)
{
    ...
    int64_t start_timeCurrent = is->ic->start_time;
    int64_t start_diff = 0;
    if (start_timeCurrent > 0 && start_timeCurrent != AV_NOPTS_VALUE)
        start_diff = fftime_to_milliseconds(start_timeCurrent);
...
long ffp_get_duration_l(FFPlayer *ffp)
{
    ...
    int64_t durationGet = fftime_to_milliseconds(is->ic->duration);
    if (durationGet < 0)
        return 0;

    return (long)durationGet;
}
...
void ffp_set_loop(FFPlayer *ffp, int loopCount)
{
    assert(ffp);
    if (!ffp)
        return;
     loop = ffp->loop = loopCount;
}
...
int ffp_queue_picture(FFPlayer *ffp, AVFrame *src_frame, double pts, double durationQueue, int64_t pos, int serial)
{
    return queue_picture(ffp, src_frame, pts, durationQueue, pos, serial);
}
