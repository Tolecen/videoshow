...
static int ijkmp_set_data_source_l(IjkMediaPlayer *mp, const char *url, int argc, char **argv)
{
    assert(mp);
    assert(url);
...
int ijkmp_set_data_source(IjkMediaPlayer *mp, const char *url, int argc, char **argv)
{
    assert(mp);
    assert(url);
    MPTRACE("ijkmp_set_data_source(url=\"%s\")\n", url);
    pthread_mutex_lock(&mp->mutex);
    int retval = ijkmp_set_data_source_l(mp, url, argc, argv);
    pthread_mutex_unlock(&mp->mutex);
...
static int ijkmp_prepare_async_l(IjkMediaPlayer *mp)
{
    ...
    int retval = ffp_prepare_async_l(mp->ffplayer, mp->data_source, mp->argc, mp->argv);
    if (retval < 0) {
