...
- (id)initWithContentURL:(NSURL *)aUrl withFilters:(NSArray *)aFilters
             withOptions:(IJKFFOptions *)options
{
    ...
    return [self initWithContentURLString:aUrlString withFilters:aFilters
                              withOptions:options];
}

- (id)initWithContentURLString:(NSString *)aUrlString withFilters:(NSArray *)aFilters
                   withOptions:(IJKFFOptions *)options
{
    if (aUrlString == nil)
        return nil;
...
- (void)playWithURLString:(NSString *)aUrlString withFilters:(NSArray *)aFilters
{
    ...
    ijkmp_set_option(_mediaPlayer, IJKMP_OPT_CATEGORY_PLAYER, "overlay-format", "fcc-rv16");
#ifdef DEBUG
...
- (void)prepareToPlay
{
    ...
    int argc = (int)_aFilters.count;
    char *argv[argc];
    
    for (int i = 0; i < argc ; i++) {
        char *argv_char = (char *)[[_aFilters objectAtIndex:i] cStringUsingEncoding:NSASCIIStringEncoding];
        //        sscanf([[arr objectAtIndex:i] UTF8String], "%x", &mCode);
        NSLog(@"char is %s in %d", argv_char, i);
        argv[i] = argv_char;
    }

    ijkmp_set_data_source(_mediaPlayer, [_urlString UTF8String], argc, argv);
...
{
    ijkmp_set_loop(_mediaPlayer, loop);
}
...
- (void)didShutdown
{
    ...
    [[NSNotificationCenter defaultCenter]
     ...
     object:self
     userInfo:@{IJKMPMoviePlayerPlaybackDidFinishReasonUserInfoKey: @(IJKMPMovieFinishReasonUserExited)}];
}
