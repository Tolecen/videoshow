...
-(void)playVideoInVisiableCells{
    ...
    LZ_TestTableViewCell *videoCell = nil;
    
    for (LZ_TestTableViewCell *cell in visiableCells) {
...
-(void)handleScrollStop{
    LZ_TestTableViewCell *bestCell = [self findTheBestToPlayVideoCell];
...
-(LZ_TestTableViewCell *)findTheBestToPlayVideoCell{
    
    // To find next cell need play video.
    // 找到下一个要播放的cell(最在屏幕中心的).
    
    LZ_TestTableViewCell *finnalCell = nil;
    NSArray *visiableCells = [self visibleCells];
