...
-(void)createMenuItems:(NSArray *)aItemsArray{
    ...
    for (NSDictionary *lDic in aItemsArray) {
        ...
        [vButton.titleLabel setFont:kFontSizeSmall];
        vButton.titleLabel.numberOfLines = 0;
        vButton.titleLabel.textAlignment = NSTextAlignmentCenter;
        vButton.titleLabel.lineBreakMode = NSLineBreakByWordWrapping;
...
#pragma mark 改变button的文字
-(void)setButtonText:(NSArray *)aItemsArray{
    for (int i = 0; i < mButtonArray.count; i++) {
        UIButton *vButton = [mButtonArray objectAtIndex:i];
        [vButton setTitle:[aItemsArray objectAtIndex:i] forState:UIControlStateNormal];
        [vButton setTitle:[aItemsArray objectAtIndex:i] forState:UIControlStateSelected];
        [mButtonArray replaceObjectAtIndex:i withObject:vButton];
    }
}
