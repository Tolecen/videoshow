...
- (void)getCameraRollAlbum:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage completion:(void (^)(TZAlbumModel *))completion{
    ...
    if (iOS8Later) {
        ...
        for (PHAssetCollection *collection in smartAlbums) {
            if ([self isAllPhoto:collection.localizedTitle]) {
...
- (void)getOriginalPhotoWithAsset:(id)asset completion:(void (^)(UIImage *photo,NSDictionary *info))completion {
    if ([asset isKindOfClass:[PHAsset class]]) {
        ...
        NSDictionary *dic = [NSDictionary dictionaryWithObjectsAndKeys:url, @"PHImageFileURLKey", nil];
        ...
        dispatch_async(dispatch_get_global_queue(0,0), ^{
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                if (completion) completion(originalImage,dic);
            });
        });
    }
}
...
- (NSString *)getNewAlbumName:(NSString *)name {
    if (iOS8Later) {
        NSString *newName;
        if ([name rangeOfString:@"Roll"].location != NSNotFound)         newName = @"所有照片";
...
- (BOOL)isAllPhoto:(NSString *)name
{
    return [name isEqualToString:@"Camera Roll"] || [name isEqualToString:@"相机胶卷"] || [name isEqualToString:@"所有照片"];
}
...
- (void)getFileFromPHAsset:(PHAsset *)asset Complete:(ResultPath)result {
    ...
    if (resource != nil) {
        [self copyFilePHAssetResource:resource Complete:result];
    } else {
        
        
        __block NSString *fileName = @"tempFile";
...
- (void)copyFilePHAssetResource:(PHAssetResource*)resource Complete:(ResultPath)result
{
...
- (void)copyFilesFromAsset:(NSArray *)assets Complete:(ResultPathMul)result
{
    NSMutableArray *filePaths = [[NSMutableArray alloc]init];
    NSMutableArray *fileNames = [[NSMutableArray alloc]init];
    
    for (id assert in assets) {
        [self copyFileFromAsset:assert Complete:^(NSString *filePath, NSString *fileName) {
            if (filePath != nil) {
