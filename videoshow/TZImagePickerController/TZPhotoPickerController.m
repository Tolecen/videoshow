...
@interface TZPhotoPickerController ()<UICollectionViewDataSource,UICollectionViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate, UITableViewDelegate, UITableViewDataSource> {
...
- (void)viewDidLoad {
    ...
    CGFloat margin = 4;
    _itemWH = (self.view.tz_width - 2 * margin - 4) / 4 - margin;
    _bottomHeight = _tzImagePickerVc.allowPickingImage ? (50 + _itemWH) : 50;
    
    self.view.backgroundColor = [UIColor whiteColor];
    self.navigationItem.title = _model.name;
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"取消" style:UIBarButtonItemStylePlain target:self action:@selector(cancel)];
    if ([[TZImageManager manager] isAllPhoto:_model.name]) {
...
- (void)configCollectionView {
    ...
    _collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(4, 0, self.view.tz_width - 2 * 4, self.view.tz_height - _bottomHeight) collectionViewLayout:layout];
...
- (void)configBottomToolBar {
    ...
    UIView *bottomToolBar = [[UIView alloc] initWithFrame:CGRectMake(0, self.view.tz_height - _bottomHeight, self.view.tz_width, _bottomHeight)];
    ...
    divide2.frame = CGRectMake(0, 50, self.view.tz_width, 1);
    ...
    [bottomToolBar addSubview:divide2];
    
    [self.view addSubview:bottomToolBar];
    
    if (_tzImagePickerVc.allowPickingImage) {
        
        _tableview = [[UITableView alloc]initWithFrame:CGRectMake(0, self.view.tz_height, self.view.tz_width, _itemWH) style:UITableViewStylePlain];
        ...
        _tableview.frame = CGRectMake(0, self.view.tz_height, self.view.tz_width, _itemWH);
        _tableview.rowHeight = _itemWH;
        
        _tableview.separatorStyle = UITableViewCellSeparatorStyleNone;
        [_tableview registerClass:[TZSelectedCell class] forCellReuseIdentifier:NSStringFromClass([TZSelectedCell class])];
...
- (void)okButtonClick {
    ...
    for (NSInteger i = 0; i < _tzImagePickerVc.selectedModels.count; i++) {
        ...
        [[TZImageManager manager] getPhotoWithAsset:model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
            ...
            if ([_tzImagePickerVc.pickerDelegate respondsToSelector:@selector(imagePickerController:didFinishPickingPhotos:sourceAssets:)]) {
                [_tzImagePickerVc.pickerDelegate imagePickerController:_tzImagePickerVc didFinishPickingPhotos:photos sourceAssets:assets];
            }
            if ([_tzImagePickerVc.pickerDelegate respondsToSelector:@selector(imagePickerController:didFinishPickingPhotos:sourceAssets:infos:)]) {
                [_tzImagePickerVc.pickerDelegate imagePickerController:_tzImagePickerVc didFinishPickingPhotos:photos sourceAssets:assets infos:infoArr];
            }
            if (_tzImagePickerVc.didFinishPickingPhotosHandle) {
                _tzImagePickerVc.didFinishPickingPhotosHandle(photos,assets);
            }
            if (_tzImagePickerVc.didFinishPickingPhotosWithInfosHandle) {
                _tzImagePickerVc.didFinishPickingPhotosWithInfosHandle(photos,assets,infoArr);
            }
...
- (void)refreshSelectTable
{
    [_tableview reloadData];
    if (self.tzImagePickerVc.selectedModels.count > 0) {
        NSIndexPath* indexPath = [NSIndexPath indexPathForRow:self.tzImagePickerVc.selectedModels.count - 1 inSection:0];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    NSLog(@"self.tzImagePickerVc.selectedModels.count : %lu", (unsigned long)self.tzImagePickerVc.selectedModels.count);
    return self.tzImagePickerVc.selectedModels.count;
    
    
//    TZImagePickerController *imagePickerVc = (TZImagePickerController *)self.navigationController;
//    return imagePickerVc.selectedModels.count;
}
...
{
    __weak typeof(self) weakSelf = self;
    TZAssetModel *model = weakSelf.tzImagePickerVc.selectedModels[indexPath.row];
//    TZAssetModel *model = _photoArr[indexPath.row];
//    __weak typeof(self) weakSelf = self;
    TZSelectedCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([TZSelectedCell class])];
    cell.cellIndex = indexPath.row;
    cell.model = model;
    cell.removeSelectedPhotoBlock = ^() {
