...
//    self.selectImageView.image = self.selectPhotoButton.isSelected ? [UIImage imageNamedFromMyBundle:@"photo_sel_photoPickerVc.png"] : [UIImage imageNamedFromMyBundle:@"photo_def_photoPickerVc.png"];
    ...
    if (self.type == TZAssetCellTypePhoto) {
        self.imageView.userInteractionEnabled = YES;
        UITapGestureRecognizer *iconTap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(selectPhotoButtonClick)];
        [self.imageView addGestureRecognizer:iconTap];
    } else {
...
- (void)selectPhotoButtonClick {
    if (self.didSelectPhotoBlock) {
        self.didSelectPhotoBlock(_model.isSelected);
    }
...
- (void)layoutSubviews {
    ...
    _selectedCountButton.frame = CGRectMake(self.tz_width - 24 - 30, 33, 24, 24);
}
...
- (UIImageView *)posterImageView {
    if (_arrowImageView == nil) {
        ...
        posterImageView.frame = CGRectMake(10, 10, 70, 70);
...
- (UILabel *)titleLable {
    if (_titleLable == nil) {
        ...
        titleLable.frame = CGRectMake(90, 0, self.tz_width - 80 - 50, self.tz_height);
...
- (UIImageView *)arrowImageView {
    if (_arrowImageView == nil) {
        ...
        CGFloat arrowWH = 15;
        arrowImageView.frame = CGRectMake(self.tz_width - arrowWH - 12, 38, arrowWH, arrowWH);
